...
termic tab <task> [--agent <cli>|--shell]     # open a NEW terminal tab in an already
           [-p <text>] [--wait]               # running task - a second claude beside the
                                              # first, or a shell tab. Same machinery as
                                              # the GUI's "+" tab menu, via the webview
                                              # RPC. Agent tabs inherit the task's cage
                                              # (pty_spawn carries task_id, so sandbox
                                              # policy is unchanged); shell tabs are
                                              # uncaged exactly like the GUI's
...
termic wait <task> [--timeout <dur>]          # block until the agent is QUIESCENT, and
                                              # quiescence is SUSTAINED, not an edge:
                                              # settled AND message queue empty, HELD
                                              # through a debounce window. Two field-
                                              # observed failure modes force both halves:
                                              # settle alone races send's queueing (turn
                                              # settles -> wait returns -> only then does
                                              # the queued prompt deliver, so `send foo
                                              # -p ".." && wait foo && diff foo` diffs
                                              # the WRONG state), and a single settle
                                              # edge fires on mid-job pauses (an agent
                                              # reported "finished" 15 rows into a 63-row
                                              # job - the heuristics pause-detect, they
                                              # do not job-detect). --wait semantics
                                              # differ by verb intent: `new --wait` IS
                                              # sustained quiescence (you want the job
                                              # done, so the right pattern is built in,
                                              # not folklore); `send --wait` tracks its
                                              # OWN prompt (delivered + that turn
                                              # settled) - mid-conversation, "my turn
                                              # finished" is the meaningful unit.
...
termic archive <task> [--yes]                 # kills the task's live PTYs FIRST (the
                                              ...
                                              # archiveTask.ts kills them - filed as
                                              # #138, an app bug independent of the CLI
                                              # that should land before Phase 1 so this
                                              # verb builds on fixed semantics).
                                              # Attached clients get in-band
                                              # {type:"detach", reason:"archived"} + a
                                              # distinct exit code
...
Multi-tab addressing: tasks can hold several terminal tabs (the GUI's tab
bar), so `send`, `wait`, `attach`, and `tab`-seeded prompts all target a tab,
not a task. The default is the task's default agent tab - the same one
work-state arming and the GUI's own prompt injection target - so the common
case needs no selector. `--tab <n|title>` overrides, using the index or
title shown by `termic status`, which lists every tab with its own
workState (per-tab `workState` already exists on `TerminalTab`, so
tab-scoped wait/send need no new state). An ambiguous title errors listing
candidates, same rule as task names.
...
- **Phase 1**: `termic new` (setup streaming, prompt injection) with
  ...
  server; exit 0 requires CONFIRMED delivery plus the verb's wait
  semantics met (sustained quiescence for `new`, own-turn settle for
  `send` - see the wait entry), and undelivered prompts exit with the
  dedicated "prompt never delivered" code.
  ...
  of heuristics.
- **Phase 2**: `termic send` (same `--wait`), `termic tab` plus the `--tab`
  selector on send/wait/attach (tab targeting ships as one unit), `termic
  apply`, `termic attach` with `--shell` (Rust-side PTY
